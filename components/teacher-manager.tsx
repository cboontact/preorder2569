...
export function TeacherManager({ onUpdate }: { onUpdate: () => Promise<void> }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const data = { role: form.superadmin ? "superadmin" : "teacher", full_name: form.full_name, advisor_grade: form.advisor_grade, advisor_room: form.advisor_room, advisor_rooms: [{grade:form.advisor_grade,room:form.advisor_room},...(form.extraRooms || [])], active: form.active, ...(form.password ? { password: form.password } : {}) };
  ...
  return <section className="panel"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-3"><FontAwesomeIcon className="h-5 w-5 text-sky-700" icon={faChalkboardUser} />จัดการครูที่ปรึกษา</h2><button className="btn" onClick={() => { setModalError(""); setForm({ ...blank }); }}><Plus size={18} />เพิ่มบัญชี</button></div>
    ...
    {form && <Modal title={form.editing ? "แก้ไขครูที่ปรึกษา" : "เพิ่มครูที่ปรึกษา"} close={() => { if (!busy) setForm(null); }}><Notice message={modalError} /><form onSubmit={save} className="grid gap-5 sm:grid-cols-2">
      ...
      <div><label htmlFor="t-role" className="label">สิทธิ์บัญชี</label><select id="t-role" className="field" value={form.superadmin ? "superadmin" : "teacher"} onChange={e => setForm({ ...form, superadmin: e.target.value === "superadmin" })}><option value="teacher">ครูที่ปรึกษา</option><option value="superadmin">ผู้ดูแลระบบ</option></select></div>
