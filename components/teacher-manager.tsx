...
type Form = typeof blank & { editing?: boolean; superadmin?: boolean; admin?: boolean; extraRooms?: AdvisorRoom[] };
export function TeacherManager({ onUpdate, canManageSuperadmin }: { onUpdate: () => Promise<void>; canManageSuperadmin: boolean }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const data = { role: form.superadmin ? "superadmin" : form.admin ? "admin" : "teacher", full_name: form.full_name, advisor_grade: form.advisor_grade, advisor_room: form.advisor_room, advisor_rooms: [{grade:form.advisor_grade,room:form.advisor_room},...(form.extraRooms || [])], active: form.active, ...(form.password ? { password: form.password } : {}) };
  ...
  return <section className="panel"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-3"><FontAwesomeIcon className="h-5 w-5 text-sky-700" icon={faChalkboardUser} />จัดการครูที่ปรึกษา</h2><button className="btn" onClick={() => { setModalError(""); setForm({ ...blank }); }}><Plus size={18} />เพิ่มบัญชี</button></div>
    ...
    {loading ? <Loading /> : <><div className="overflow-x-auto"><table className="data-table"><thead><tr><TableHeading label="ครู / ชื่อผู้ใช้" /><TableHeading label="ห้องที่ปรึกษา" /><TableHeading label="สิทธิ์" /><TableHeading label="สถานะ" /><TableHeading label="จัดการ" /></tr></thead><tbody>{filtered.map(t => <tr key={t.username}><td className="min-w-48 font-medium">{t.full_name}<span className="muted block">{t.username}</span></td><td>{t.advisor_rooms.map(r=>`${r.grade}/${r.room}`).join(", ")}</td><td className="whitespace-nowrap">{t.role === "superadmin" ? "ผู้ดูแลสูงสุด" : t.role === "admin" ? "ผู้ดูแลระบบ" : "ครูที่ปรึกษา"}</td><td className="whitespace-nowrap">{t.active ? "ใช้งาน" : "ปิดใช้งาน"}</td><td><div className="flex gap-2">{(t.role !== "superadmin" || canManageSuperadmin) && <button className="btn-secondary h-11 w-11 p-0" title="แก้ไขบัญชี" aria-label={`แก้ไข ${t.full_name}`} onClick={() => { setModalError(""); setForm({ username: t.username, full_name: t.full_name, advisor_grade: t.advisor_grade, advisor_room: t.advisor_room, active: !!t.active, password: "", extraRooms: t.advisor_rooms.filter(r=>r.grade!==t.advisor_grade || r.room!==t.advisor_room), editing: true, superadmin: t.role === "superadmin", admin: t.role === "admin" }); }}><Pencil size={17} /></button>}{t.role !== "superadmin" && <button className="btn-danger h-11 w-11 p-0" title="ลบบัญชี" aria-label={`ลบ ${t.full_name}`} onClick={() => { setModalError(""); setDeletion(t); }}><Trash2 size={16} /></button>}</div></td></tr>)}</tbody></table></div>{!filtered.length && <Empty text="ไม่พบครูที่ปรึกษา" />}</>}
    {form && <Modal title={form.editing ? "แก้ไขครูที่ปรึกษา" : "เพิ่มครูที่ปรึกษา"} close={() => { if (!busy) setForm(null); }}><Notice message={modalError} /><form onSubmit={save} className="grid gap-5 sm:grid-cols-2">
      ...
      <div><label htmlFor="t-password" className="label">{form.editing ? "ตั้งรหัสผ่านใหม่ (ถ้าต้องการ)" : "รหัสผ่าน"}</label><input id="t-password" className="field" type="password" autoComplete="new-password" required={!form.editing} minLength={1} maxLength={256} value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /><p className="muted mt-1">กรอกได้ตั้งแต่ 1 ตัวอักษร{form.editing ? " · เว้นว่างเพื่อใช้รหัสเดิม" : ""}</p></div>
      <div><label htmlFor="t-role" className="label">สิทธิ์บัญชี</label><select id="t-role" className="field" value={form.superadmin ? "superadmin" : form.admin ? "admin" : "teacher"} onChange={e => setForm({ ...form, superadmin: e.target.value === "superadmin", admin: e.target.value === "admin" })}><option value="teacher">ครูที่ปรึกษา</option><option value="admin">ผู้ดูแลระบบ</option>{canManageSuperadmin && <option value="superadmin">ผู้ดูแลสูงสุด</option>}</select></div>
      ...
      <button className="btn sm:col-span-2" disabled={busy}><ActionSave className="mr-1.5" />{busy ? "กำลังบันทึก…" : "บันทึกบัญชี"}</button>
    </form></Modal>}
